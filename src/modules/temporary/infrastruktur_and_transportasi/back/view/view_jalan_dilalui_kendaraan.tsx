...
export default function ViewJalanDilaluiKendaraan({ provinsi, download, table }: { provinsi: any, download: any, table: any }) {
    ...
    async function onProses() {
        const loadData = await funGetJalanDilaluiKendaraan({ provinsi: isProvinsi, kab: isKabupaten })
    ...
    return (
        <>
            <Stack>
                <Text fw={"bold"}>TRANSPORTASI - JALAN DARAT DAPAT DILALUI KENDARAAN</Text>
            </Stack>
            <Box pt={30}>
                <SimpleGrid
            ...
            <Box pt={30}>
                <TableJalanDilaluiKendaraan data={dataTable.data} title={dataTable.title} th={dataTable.thTitle} />
            </Box>
        </>
    )
}