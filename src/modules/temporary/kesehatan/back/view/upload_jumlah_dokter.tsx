...
export default function ViewUploadJumlahDokter() {
    ...
    async function onLoad(data: any) {
        if (data.length > 0) {
            ...
            if (('id' in data[0]) &&
                ...
                ('Kabkot' in data[0]) &&
                ('Kecamatan' in data[0]) &&
                ('TenagaDokterPriaYangTinggalMenetapDiDesa' in data[0]) &&
                ('TenagaDokterWanitaYangTinggalMenetapDiDesa' in data[0])) {
    ...
    return (
        <>
            ...
            <Stack p={"md"}>
                ...
                >
                    <Text fw={"bold"} c={"white"} mb={20}>UPLOAD DATA JUMLAH DOKTER</Text>
                    <Dropzone
                        loading={isLoading}
                        style={{
                            border: "1px dashed",
                                            ...
                                            >
                                                <Table.Thead>
                                                    ...
                                                    >
                                                        ...
                                                        <Table.Th>Dokter Pria Yang Menetap Di Desa</Table.Th>
                                                        <Table.Th>Dokter Wanita Yang Menetap Di Desa</Table.Th>
                                                    </Table.Tr>
                                                </Table.Thead>
                                                <Table.Tbody>
                                                    {json.map(home =>
                                                        <Table.Tr key={home.id}>
                                                            ...
                                                            <Table.Td>{home.TenagaDokterPriaYangTinggalMenetapDiDesa}</Table.Td>
                                                            <Table.Td>{home.TenagaDokterWanitaYangTinggalMenetapDiDesa}</Table.Td>
                                                        </Table.Tr>
                                                    )}
                                                </Table.Tbody>
                                            </Table>
            ...
            >
                <ModalUploadKesehatan kategori={"jumlah dokter"} data={json} onSuccess={(val) => {
                    setJson([])
                }} />
            </Modal>
        </>
