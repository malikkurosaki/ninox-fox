...
import ModalUploadKesehatan from "../component/modal_kesehatan"

export default function ViewUploadRataRataJarakFasilitas() {
    ...
    async function onLoad(data: any) {
        if (data.length > 0) {
            ...
            if (('id' in data[0]) &&
                ...
                ('Kabkot' in data[0]) &&
                ('Kecamatan' in data[0]) &&
                ('JarakTerdekatMenujuBidan' in data[0]) &&
                ('JarakTerdekatMenujuPuskesmasTanpaRawatInap' in data[0]) &&
                ('JarakTerdekatMenujuPuskesmasDenganRawatInap' in data[0]) &&
                ('JarakTerdekatMenujuRumahSakit' in data[0])) {
    ...
    return (
        <>
            ...
            <Stack p={"md"}>
                ...
                >
                    <Text fw={"bold"} c={"white"} mb={20}>UPLOAD DATA RATA-RATA JARAK KE FASILITAS KESEHATAN</Text>
                    <Dropzone
                        loading={isLoading}
                        style={{
                            border: "1px dashed",
                                            ...
                                            >
                                                <Table.Thead>
                                                    ...
                                                    >
                                                        ...
                                                        <Table.Th>Bidan</Table.Th>
                                                        <Table.Th>Puskesmas Tanpa Rawat Inap</Table.Th>
                                                        <Table.Th>Puskesmas Dengan Rawat Ianp</Table.Th>
                                                ...
                                                <Table.Tbody>
                                                    {json.map(home =>
                                                        <Table.Tr key={home.id}>
                                                            ...
                                                            <Table.Td>{home.JarakTerdekatMenujuBidan}</Table.Td>
                                                            <Table.Td>{home.JarakTerdekatMenujuPuskesmasTanpaRawatInap}</Table.Td>
                                                            <Table.Td>{home.JarakTerdekatMenujuPuskesmasDenganRawatInap}</Table.Td>
                                                            <Table.Td>{home.JarakTerdekatMenujuRumahSakit}</Table.Td>
                                                        </Table.Tr>
                                                    )}
                                                </Table.Tbody>
                                            </Table>
            ...
            >
                <ModalUploadKesehatan kategori={"rata rata jarak fasilitas"} data={json} onSuccess={(val) => {
                    setJson([])
                }} />
            </Modal>
        </>
