...
export default function ViewUploadPosPelayanan() {
    ...
    async function onLoad(data: any) {
        if (data.length > 0) {
            ...
            if (('id' in data[0]) &&
                ...
                ('Kabkot' in data[0]) &&
                ('Kecamatan' in data[0]) &&
                ('JumlahPosPembinaanTerpadu' in data[0]) &&
                ('JumlahPosyanduAktif' in data[0])) {
    ...
    return (
        <>
            ...
            <Stack p={"md"}>
                ...
                >
                    <Text fw={"bold"} c={"white"} mb={20}>UPLOAD DATA JUMLAH POS PELAYANAN KESEHATAN</Text>
                    <Dropzone
                        loading={isLoading}
                        style={{
                            border: "1px dashed",
                                            ...
                                            >
                                                <Table.Thead>
                                                    ...
                                                    >
                                                        ...
                                                        <Table.Th>Pos Pembinaan Terpadu</Table.Th>
                                                        <Table.Th>Posyandu Aktif</Table.Th>
                                                    </Table.Tr>
                                                </Table.Thead>
                                                <Table.Tbody>
                                                    {json.map(home =>
                                                        <Table.Tr key={home.id}>
                                                            ...
                                                            <Table.Td>{home.JumlahPosPembinaanTerpadu}</Table.Td>
                                                            <Table.Td>{home.JumlahPosyanduAktif}</Table.Td>
                                                        </Table.Tr>
                                                    )}
                                                </Table.Tbody>
                                            </Table>
            ...
            >
                <ModalUploadKesehatan kategori={"pos pelayanan"} data={json} onSuccess={(val) => {
                    setJson([])
                }} />
            </Modal>
        </>
