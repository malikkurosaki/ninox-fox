...
export default async function funGetJaminanUntukBaduta({ provinsi, kab }: { provinsi?: any, kab?: any }) {
    ...
    if (!_.isNull(provinsi) && !_.isUndefined(provinsi) && provinsi > 0 && provinsi <= nProv) {
        if (_.isNull(kab) || _.isUndefined(kab)) {
            dataTable = await prisma.sE_Kesehatan_JaminanUntukBaduta.findMany({
                where: {
