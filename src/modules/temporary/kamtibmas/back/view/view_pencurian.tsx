...
export default function ViewPencurian({ provinsi, download, table }: { provinsi: any, download: any, table: any }) {
    ...
    async function onProses() {
        const loadData = await funGetPencurian({ provinsi: isProvinsi, kab: isKabupaten })
    ...
    return (
        <>
            <Stack>
                <Text fw={"bold"}>KEAMANAN - INTENSITAS PENCURIAN</Text>
            </Stack>
            <Box pt={30}>
                <SimpleGrid
            ...
            <Box pt={30}>
                <TablePencurian data={dataTable.data} title={dataTable.title} th={dataTable.thTitle} />
            </Box>
        </>
    )
}