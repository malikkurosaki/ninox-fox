...
export default function ViewPenganiayaan({ provinsi, download, table }: { provinsi: any, download: any, table: any }) {
    ...
    async function onProses() {
        const loadData = await funGetPenganiayaan({ provinsi: isProvinsi, kab: isKabupaten })
    ...
    return (
        <>
            <Stack>
                <Text fw={"bold"}>KEAMANAN - INTENSITAS KEJAHATAN PENGANIAYAAN</Text>
            </Stack>
            <Box pt={30}>
                <SimpleGrid
            ...
            <Box pt={30}>
                <TablePenganiayaan data={dataTable.data} title={dataTable.title} th={dataTable.thTitle} />
            </Box>
        </>
    )
}