...
export default async function funDownloadPenganiayaan() {
    const data = await prisma.sE_Keamanan_Penganiayaan.findMany({
        select: {
            id: true,
            idProvinsi: true,
            idKabkot: true,
    ...
    const allData = {
        title: 'Keamanan - Intensitas Kejahatan Penganiayaan',
        data: dataTable
    }
