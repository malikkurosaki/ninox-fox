...
export default async function funDownloadPencurianDanKekerasan() {
    const data = await prisma.sE_Keamanan_PencurianDanKekerasan.findMany({
        select: {
            id: true,
            idProvinsi: true,
            idKabkot: true,
    ...
    const allData = {
        title: 'Keamanan - Intensitas Pencurian Dan Kekerasan',
        data: dataTable
    }
