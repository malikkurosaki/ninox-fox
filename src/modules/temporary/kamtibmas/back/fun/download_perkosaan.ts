...
export default async function funDownloadPerkosaan() {
    const data = await prisma.sE_Keamanan_Perkosaan.findMany({
        select: {
            id: true,
            idProvinsi: true,
            idKabkot: true,
    ...
    const allData = {
        title: 'Keamanan - Intensitas Kejahatan Perkosaan/Asusila',
        data: dataTable
    }
