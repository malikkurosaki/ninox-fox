...
export default async function funUploadJaminanPensiun({ body }: { body: any }) {

    // looping data csv
    for (let i of body) {
        // update data audience
        await prisma.sE_Ketenagakerjaan_JaminanPensiun.update({
            where: {
                id: Number(i.id)
            },
            data: {
    ...
    revalidatePath('dashboard/se/jaminan-pensiun')
