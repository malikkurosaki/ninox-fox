...
export default async function funGetGuruTersertifikasi({ provinsi, kab }: { provinsi?: any, kab?: any }) {
    ...
    if (!_.isNull(provinsi) && !_.isUndefined(provinsi) && provinsi > 0 && provinsi <= nProv) {
        if (_.isNull(kab) || _.isUndefined(kab)) {
            dataTable = await prisma.sE_Pendidikan_GuruTersertifikasi.findMany({
                where: {
