...
export default function ViewGuruHonorer({ provinsi, download, table }: { provinsi: any, download: any, table: any }) {
    ...
    async function onProses() {
        const loadData = await funGetGuruHonorer({ provinsi: isProvinsi, kab: isKabupaten })
    ...
    return (
        <>
            <Stack>
                <Text fw={"bold"}>PENDIDIKAN - GURU HONORER</Text>
            </Stack>
            <Box pt={30}>
                <SimpleGrid
            ...
            <Box pt={30}>
                <TableGuruHonorer data={dataTable.data} title={dataTable.title} th={dataTable.thTitle} />
            </Box>
        </>
    )
}