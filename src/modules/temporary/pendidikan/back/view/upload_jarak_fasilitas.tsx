...
import ModalUploadPendidikan from "../component/modal_pendidikan"

export default function ViewUploadJarakFasilitas() {
    ...
    async function onLoad(data: any) {
        if (data.length > 0) {
            ...
            if (('id' in data[0]) &&
                ...
                ('Kabkot' in data[0]) &&
                ('Kecamatan' in data[0]) &&
                ('JarakKeSDTerdekat' in data[0]) &&
                ('JarakKeSMPTerdekat' in data[0]) &&
                ('JarakKeSMATerdekat' in data[0]) &&
                ('JarakKeSMKTerdekat' in data[0])) {
    ...
    return (
        <>
            ...
            <Stack p={"md"}>
                ...
                >
                    <Text fw={"bold"} c={"white"} mb={20}>UPLOAD DATA RATA-RATA JARAK KE FASILITAS PENDIDIKAN</Text>
                    <Dropzone
                        loading={isLoading}
                        style={{
                            border: "1px dashed",
                                            ...
                                            >
                                                <Table.Thead>
                                                    ...
                                                    >
                                                        ...
                                                        <Table.Th>Jarak ke SD</Table.Th>
                                                        <Table.Th>Jarak ke SMP</Table.Th>
                                                        <Table.Th>Jarak ke SMA</Table.Th>
                                                        <Table.Th>Jarak ke SMK</Table.Th>
                                                    </Table.Tr>
                                                </Table.Thead>
                                                <Table.Tbody>
                                                    {json.map(home =>
                                                        <Table.Tr key={home.id}>
                                                            ...
                                                            <Table.Td>{home.JarakKeSDTerdekat}</Table.Td>
                                                            <Table.Td>{home.JarakKeSMPTerdekat}</Table.Td>
                                                            <Table.Td>{home.JarakKeSMATerdekat}</Table.Td>
                                                            <Table.Td>{home.JarakKeSMKTerdekat}</Table.Td>
                                                        </Table.Tr>
                                                    )}
                                                </Table.Tbody>
                                            </Table>
            ...
            >
                <ModalUploadPendidikan kategori={"jarak fasilitas"} data={json} onSuccess={(val) => {
                    setJson([])
                }} />
            </Modal>
        </>
