...
export default function ViewBpjs({ provinsi, download, table }: { provinsi: any, download: any, table: any }) {
    ...
    async function onProses() {
        const loadData = await funGetBpjs({ provinsi: isProvinsi, kab: isKabupaten })
    ...
    return (
        <>
            <Stack>
                <Text fw={"bold"}>KEMISKINAN - PESERTA BPJS KESEHATAN</Text>
            </Stack>
            <Box pt={30}>
                <SimpleGrid
            ...
            <Box pt={30}>
                <TableBpjs data={dataTable.data} title={dataTable.title} th={dataTable.thTitle} />
            </Box>
        </>
    )
}