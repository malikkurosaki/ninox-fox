...
import ModalUploadEkonomi from "../component/modal_ekonomi"

export default function ViewUploadLembagaKeuangan() {
    ...
    async function onLoad(data: any) {
        if (data.length > 0) {
            ...
            if (('id' in data[0]) &&
                ...
                ('Kabkot' in data[0]) &&
                ('Kecamatan' in data[0]) &&
                ('JumlahBankUmumPemerintah' in data[0]) &&
                ('JumlahBankUmumSwasta' in data[0]) &&
                ('JumlahBankPengkreditanRakyat' in data[0]) &&
                ('JumlahKoperasiSimpanPinjam' in data[0])) {
    ...
    return (
        <>
            ...
            <Stack p={"md"}>
                ...
                >
                    <Text fw={"bold"} c={"white"} mb={20}>UPLOAD DATA KETERSEDIAAN LEMBAGA KEUANGAN</Text>
                    <Dropzone
                        loading={isLoading}
                        style={{
                            border: "1px dashed",
                                            ...
                                            >
                                                <Table.Thead>
                                                    ...
                                                    >
                                                        ...
                                                        <Table.Th>Jumlah Bank Umum Pemerintah</Table.Th>
                                                        <Table.Th>Jumlah Bank Umum Swasta</Table.Th>
                                                        <Table.Th>Jumlah Bank Pengkreditan Rakyat</Table.Th>
                                                        <Table.Th>Jumlah Koperasi Simpan Pinjam</Table.Th>
                                                    </Table.Tr>
                                                </Table.Thead>
                                                <Table.Tbody>
                                                    {json.map(home =>
                                                        <Table.Tr key={home.id}>
                                                            ...
                                                            <Table.Td>{home.JumlahBankUmumPemerintah}</Table.Td>
                                                            <Table.Td>{home.JumlahBankUmumSwasta}</Table.Td>
                                                            <Table.Td>{home.JumlahBankPengkreditanRakyat}</Table.Td>
                                                            <Table.Td>{home.JumlahKoperasiSimpanPinjam}</Table.Td>
                                                        </Table.Tr>
                                                    )}
                                                </Table.Tbody>
                                            </Table>
            ...
            >
                <ModalUploadEkonomi kategori={"lembaga keuangan"} data={json} onSuccess={(val) => {
                    setJson([])
                }} />
            </Modal>
        </>
