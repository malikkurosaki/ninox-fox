...
export default function ViewUploadJumlahPasar() {
    ...
    async function onLoad(data: any) {
        if (data.length > 0) {
            ...
            if (('id' in data[0]) &&
                ...
                ('Kabkot' in data[0]) &&
                ('Kecamatan' in data[0]) &&
                ('JumlahPasarDenganBangunanPermanen' in data[0]) &&
                ('JumlahPasarDenganBangunanSemiPermanen' in data[0]) &&
                ('JumlahPasarTanpaBangunan' in data[0])) {
    ...
    return (
        <>
            ...
            <Stack p={"md"}>
                ...
                >
                    <Text fw={"bold"} c={"white"} mb={20}>UPLOAD DATA JUMLAH PASAR BERDASARKAN JENIS BANGUNAN</Text>
                    <Dropzone
                        loading={isLoading}
                        style={{
                            border: "1px dashed",
                                            ...
                                            >
                                                <Table.Thead>
                                                    ...
                                                    >
                                                        ...
                                                        <Table.Th>Pasar Dengan Bangunan Permanen</Table.Th>
                                                        <Table.Th>Pasar Dengan Bangunan Semi Permanen</Table.Th>
                                                        <Table.Th>Pasar Tanpa Bangunan</Table.Th>
                                                    </Table.Tr>
                                                </Table.Thead>
                                                <Table.Tbody>
                                                    {json.map(home =>
                                                        <Table.Tr key={home.id}>
                                                            ...
                                                            <Table.Td>{home.JumlahPasarDenganBangunanPermanen}</Table.Td>
                                                            <Table.Td>{home.JumlahPasarDenganBangunanSemiPermanen}</Table.Td>
                                                            <Table.Td>{home.JumlahPasarTanpaBangunan}</Table.Td>
                                                        </Table.Tr>
                                                    )}
                                                </Table.Tbody>
                                            </Table>
            ...
            >
                <ModalUploadEkonomi kategori={"jumlah pasar"} data={json} onSuccess={(val) => {
                    setJson([])
                }} />
            </Modal>
        </>
